...
Against the three-seed attention mean the one-run no-attention model is
nominally better on all four metrics, by \(1.1\), \(2.9\), \(1.6\) and
\(0.8\) standard deviations of the attention arm respectively, with PSNR
and SSIM falling outside the observed three-seed range. A single run cannot
establish superiority, and the comparison is confounded because the
no-attention run shares its seed with the strongest of the three attention
runs. We therefore record only that attention is not required to obtain the
result, consistent with the \(0.13\%\) missing-region validation gap and
the earlier three-seed ablation (\(+0.10\%\)).
...
\bottomrule
\end{tabular}
\caption{Selected complexity, spectral, and controllability diagnostics.  Resistance-based, spanning-tree, leading-eigenvalue, and average-controllability quantities are close, while the random-walk eigengap remains different.}
