...
\noindent
This Supporting Information contains the detailed construction procedures,
...
The Supporting Information gives reproducibility details for those models and
...
A mechanism-check variant removes only the three supernode-attention modules.
The graph, collector, recurrent update, readout, encoder, decoder, data
pipeline, and trainer are otherwise unchanged.

The graph-plus-attention model was trained and evaluated with three
independent seeds. The no-attention variant was evaluated in only one run on
the final substrate. It is therefore not treated as a separately replicated
model arm; it is used only as a mechanism check to determine whether removing
the attention blocks produces a large change in performance.
...
\paragraph{Attention mechanism check.}
The single no-attention run gives, on benchmark masks,
\[
\mathrm{L1}=1.4507\%,\qquad
\mathrm{PSNR}=29.843,\qquad
\mathrm{SSIM}=0.90415,\qquad
\mathrm{LPIPS}=0.09332.
\]
For comparison, the three-seed graph-plus-attention result is
\[
\mathrm{L1}=1.4533\pm0.0024\%,\qquad
\mathrm{PSNR}=29.820\pm0.008,
\]
\[
\mathrm{SSIM}=0.90387\pm0.00017,\qquad
\mathrm{LPIPS}=0.09356\pm0.00032.
\]

The no-attention values are close to the attention-model results, with
differences comparable in scale to the seed-to-seed variation observed across
the three attention runs. The direction of the small difference is not
consistent across metrics. We therefore do not detect a substantial
attention-dependent performance effect in this mechanism check. Because the
no-attention condition contains only one run, we do not interpret this
comparison as a replicated equivalence test and do not claim superiority in
either direction.
...
Off-diagonal complexity & \(5.318\) & \(3.922\)\\
Degree entropy & \(5.410\) & \(7.798\)\\
\(\lambda_{\max}/\langle d\rangle\) & \(1.288\) & \(1.285\)\\
Hoffman bound & \(6.612\) & \(7.908\)\\
Perron eigengap & \(0.0938\) & \(0.0197\)\\
\(\log\tau(G)/n\) & \(4.791\) & \(4.774\)\\
Average controllability & \(1.0932\) & \(1.1004\)\\
\bottomrule
\end{tabular}
\caption{Selected complexity, spectral, and controllability diagnostics.  Resistance-based, spanning-tree, leading-eigenvalue, and average-controllability quantities are close, while off-diagonal complexity, degree entropy, and the Perron eigengap remain different.}
