...
f = 0.0 reproduces the deposited PARENT exactly (checked: 477,584 edges, 0
monochromatic, chi = 3).  It no longer reproduces the deposited QUOTIENT edge
count of 64,760: close_relaxed now permutes its candidate array before applying
the quota (see the comment there), and rng.shuffle consumes draws, so the
random stream differs from the one that produced the deposit.  At seed 99 the
quotient is 1,015 / 64,491, inside the 61,489-64,491 range of the five f = 0
seeds.  The exact 64,760 was a property of one random stream, not of the
construction; the deposited graph itself comes from construct.py and is
unaffected.
"""
...
def close_relaxed(u_arr, v_arr, n, colour, block, quota, rng, beta, f):
    ...
    for _ in range(24):
        ...
        # np.unique returns SORTED keys and key = min*n + max, so [:remaining]
        # would keep the lowest-indexed lower endpoints -- and colour and block
        # are both read off the leading base-S digit of the index, so that cut
        # is correlated with BOTH labels (it discards whole high-index blocks).
        # Permute before truncating so the quota takes a uniform sample of the
        # legal candidates.  Inert whenever len(keys) <= remaining.
        keys = np.array([k for k in keys if k not in seen], np.int64)
        rng.shuffle(keys)
        keys = keys[:remaining]
...
#: closures accepted at each stage by the most recent grow() call, in order.
#: grow() overwrites this; run() copies it into the result row.
LAST_STAGE_CLOSURES = []
...
if __name__ == '__main__':
    ...
    ap.add_argument('--control', action='store_true',
                    help='run only f=0 seed=99 and assert it reproduces the '
                         'deposited PARENT, and that the quotient lands in the '
                         'f=0 seed range (about 25 seconds)')
    ...
    if args.control:
        # The PARENT is the invariant worth asserting: at f=0 the colour filter
        # holds every closure pool at or under its quota except on end-of-quota
        # remainders, so all three stage quotas are met and the edge count is
        # forced.  The QUOTIENT edge count is not an invariant -- it depends on
        # the random stream, which the shuffle in close_relaxed moves -- so it
        # is checked against the five-seed f=0 range rather than for equality
        # with the deposited 64,760.  See the module docstring.
        F0_QUOTIENT_RANGE = (61489, 64491)   # five f=0 seeds, 2026-09-23 rerun
        r = run(0.0, 99)
        parent_ok = (r['parent_m'] == 477584 and r['mono_parent'] == 0
                     and r['n'] == 1015)
        quotient_ok = F0_QUOTIENT_RANGE[0] <= r['m'] <= F0_QUOTIENT_RANGE[1]
        ...
        print(f"REPRODUCES THE DEPOSITED PARENT (477,584 / 0 mono / 1,015): "
              f"{parent_ok}")
        print(f"QUOTIENT IN THE f=0 SEED RANGE {F0_QUOTIENT_RANGE[0]:,}"
