"""
Train feeder1004 — the v1003 ABLATION: identical model with the supernode
attention removed (attn_1/attn_2/attn_3 not constructed, their three calls in
the recurrent loop commented out). 12,864 fewer params, 0.017% of the model.

Copied verbatim from train_v1003super.py on 2026-08-26 and changed in exactly
two places, both commented in situ:
    1. the model import  -> feeder1004
    2. the --save-dir default -> ckpt_feeder1004_s0
Everything else -- loss, LR groups, schedule, data pipeline, seeding, the
rolling-checkpoint logic -- is byte-identical to the v1003 trainer, so the
comparison against ckpt_v1003_s0_r4 / _s1 / _s2 is controlled.

Note on LR groups: `brain_names` (~line 476) still lists 'attn_1', 'attn_2',
'attn_3'. Those entries simply never match now, since the modules do not
exist. Left in place deliberately -- removing them would be a third
difference from the v1003 trainer for no behavioural gain. Verified: no other
parameter changes group.

Usage:
    python -u train_feeder1004.py --seed 0 --gpu cuda:1 --from-scratch \
        --save-dir "/media/vahid/T7 Shield/ckpt_feeder1004_s0"
...
Always pass a fresh --save-dir per seed. Never reuse one.
"""
...
# ---- CHANGED 2026-08-26 (feeder1004 ablation) ----
# was: from model_v1003super import RecurrentBrainNetV7
from feeder1004 import RecurrentBrainNetV7
...
def main():
    ...
    parser.add_argument('--graph', type=str,
                        ...
                        # was: '..','graph_brain_mild_v146_feeder_supernode.npz' (superseded).
    ...
    parser.add_argument('--save-dir', type=str,
                        ...
                        # ---- CHANGED 2026-08-26 (feeder1004 ablation) ----
                        # ckpt_v1003_s0_r4 holds the finished 50-epoch v1003
                        # seed-0 run. Writing there would destroy it. This
                        # default is seed 0 only -- pass --save-dir explicitly
                        # for seeds 1 and 2.
                        # was: default='/media/vahid/T7 Shield/ckpt_v1003_s0_r4')
                        default='/media/vahid/T7 Shield/ckpt_feeder1004_s0')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        ckpt = {
            ...
            # cost is redoing the part of that epoch already done" was therefore
            # wrong: feeder1004 seed 2, resumed at ep16 on 2026-08-28, replayed
            # the shuffle orders and masks of its own ep0-15 in ep16-31 and saw
            # 34 distinct data streams over its 50 epochs instead of 50.
            # Verified by comparing DataLoader permutation sequences across a
            # simulated restart; the first epoch after a restart reproduces
            # epoch 0 exactly. Saving the generator state makes a resume an
            # actual continuation. Cannot repair checkpoints written before
            # this date -- they do not carry the state (hence the None guard on
            # the restore side).
        ...
        # ---- REMOVED 2026-08-30 ----
        # A checkpoint-retention block lived here (KEEP_RECENT=3: keep epoch 0,
        # the current best and the last 3, delete the rest). It was added
        # 2026-08-26 because three seeds x 45 GB would not fit in 73 GB free.
        # It only ever deleted files after they were written, so it could not
        # affect training -- but it was a third difference from
        # train_v1003super.py, and it destroyed ckpt_feeder1004_s2's
        # rolling_ep15_end.pt, the last clean checkpoint before the 2026-08-28
        # crash, which is what forced a 40 h from-scratch rerun instead of a
        # ~13 h resume. Removed so this trainer differs from the v1003 trainer
        # in the model import alone. Every epoch is now kept: ~48 GB per run.
