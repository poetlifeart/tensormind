...
BACK-PORTED 2026-09-25 from python_programs/tensor8k: the substrate-size tables
COLLECTOR_GRIDS / READOUT_GRIDS below.  Before this, the collector and readout
block sizes were hard-coded for the 7-vertex-seed lineage (colour classes
4,802 / 4,802 / 7,203), so on the 8,000-node seed-encoded substrate
(2,400 / 2,400 / 3,200) BrainCollectorV7 computed n_s0 = 2400 - 4352 = -1952 and
the forward pass raised "size of tensor a (4802) must match tensor b (2400)".
__init__ and the parameter count looked correct, which is why it was not caught
earlier: the slot sum is still n1, and the model prints 74,911,834 params on the
8k graph either way.

The 4,802 / 7,203 path is unchanged, bit for bit -- verified 2026-09-25 by
comparing a seeded forward on graph_degmatch_parent_supernode.npz before and
after this change: 75,965,171 params, recon sha256 prefix be2ce92001809ac3,
recon sum 71857.9989505847 in both.  Two mechanisms keep it exact:
adaptive_avg_pool2d to a level's native size is the identity, and the t3
resampling branch is guarded by `if self.grid_t3 != (32, 32)`, which is false at
n3 = 7203.
...
class RecurrentBrainNetV7(nn.Module):
    ...
    def __init__(self, graph_path, brain_channels=32, graph_init_scale=5.0, n_iters=5):
        ...
        print(f"RecurrentBrainNetV1003Super: {n_params:,} params total, "
