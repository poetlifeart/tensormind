"""
RecurrentBrainNet V1003Super  (created 2026-08-15, revised 2026-08-16)

Variant of V1001Super. Changes relative to v1001, as the code now stands:
...
class BundleSparseLinear(nn.Module):
    """Converge and fan out along the bundle map.

    A colour-to-colour bundle between two supernodes holds W fine edges.  It is
    not one channel and it is not W channels -- it reduces to

        n(W) = round( W**0.05 * (1 + (W-1)/11)**0.95 )        (at least 1)

    axons, the rule measured against the reference fibre-bundle distribution.
    The bundle's edges are dealt round-robin among its n(W) axons; each axon
    averages the distinct source vertices it was dealt, is scaled by its own
    learnable weight, and fans that value to the distinct target vertices it was
    dealt.

    Each layer is one ordered colour pair, so a bundle is keyed by
    (source supernode, target supernode) within the layer -- nothing is
    averaged across colour classes.
    """
    ...
    @staticmethod
    def _n_axons(W):
        n = torch.round(W**BundleSparseLinear.P_EXP
                        * (1.0 + (W - 1.0) / BundleSparseLinear.C_KNEE)**(1.0 - BundleSparseLinear.P_EXP))
        return torch.clamp(n, min=1.0).long()

    def __init__(self, in_nodes, out_nodes, mask, sid_in, sid_out, n_super):
        super().__init__()
        self.in_nodes, self.out_nodes = in_nodes, out_nodes
        idx = mask.nonzero(as_tuple=False)
        row = idx[:, 0].long()                        # target, indexes out_nodes
        col = idx[:, 1].long()                        # source, indexes in_nodes
        key = sid_in[col] * n_super + sid_out[row]    # the bundle
        _, rel = torch.unique(key, return_inverse=True)
        n_rel = int(rel.max().item()) + 1

        W = torch.zeros(n_rel).scatter_add_(0, rel, torch.ones_like(rel, dtype=torch.float))
        n_ax = self._n_axons(W)                       # axons per bundle
        offset = torch.cat([torch.zeros(1, dtype=torch.long), n_ax.cumsum(0)[:-1]])
        self.n_axons = int(n_ax.sum().item())
        self.n_rel = n_rel

        # deal each bundle's edges round-robin among its axons
        order = torch.argsort(rel, stable=True)
        within = torch.empty_like(order)
        within[order] = torch.arange(len(order)) - torch.cumsum(
            torch.nn.functional.pad(torch.bincount(rel, minlength=n_rel), (1, 0)), 0)[rel][order]
        axon = offset[rel] + (within % n_ax[rel])

        cin  = torch.unique(torch.stack([axon, col], 1), dim=0)   # distinct (axon, source)
        cout = torch.unique(torch.stack([axon, row], 1), dim=0)   # distinct (axon, target)
        self.register_buffer('in_ax',  cin[:, 0].contiguous())
        self.register_buffer('in_src', cin[:, 1].contiguous())
        self.register_buffer('out_ax', cout[:, 0].contiguous())
        self.register_buffer('out_tgt', cout[:, 1].contiguous())
        fan = torch.zeros(self.n_axons).scatter_add_(
            0, self.in_ax, torch.ones_like(self.in_ax, dtype=torch.float))
        self.register_buffer('inv_fan', 1.0 / fan.clamp(min=1.0))

        self.weight = nn.Parameter(torch.empty(self.n_axons))
        nn.init.uniform_(self.weight,
                         -1.0 / math.sqrt(in_nodes),
                          1.0 / math.sqrt(in_nodes))
    ...
    def forward(self, x):
        with torch.amp.autocast('cuda', enabled=False):
            x = x.float()
            B, _, C = x.shape
            axon = torch.zeros(B, self.n_axons, C, device=x.device, dtype=torch.float32)
            axon.scatter_add_(1, self.in_ax.view(1, -1, 1).expand(B, -1, C), x[:, self.in_src, :])
            axon = axon * self.inv_fan.view(1, -1, 1) * self.weight.float().view(1, -1, 1)
            out = torch.zeros(B, self.out_nodes, C, device=x.device, dtype=torch.float32)
            out.scatter_add_(1, self.out_tgt.view(1, -1, 1).expand(B, -1, C), axon[:, self.out_ax, :])
            return out
...
class RecurrentBrainNetV1004Bundle(nn.Module):
    ...
    def __init__(self, graph_path, brain_channels=32, graph_init_scale=5.0, n_iters=5):
        ...
        S1, S2, S3, NS = supernode_id_1, supernode_id_2, supernode_id_3, n_supernodes
        self.layer_12 = BundleSparseLinear(n1, n2, mask_12, S1, S2, NS)
        self.layer_13 = BundleSparseLinear(n1, n3, mask_13, S1, S3, NS)
        self.layer_23 = BundleSparseLinear(n2, n3, mask_23, S2, S3, NS)
        self.layer_32 = BundleSparseLinear(n3, n2, mask_23.t().contiguous(), S3, S2, NS)
        self.layer_31 = BundleSparseLinear(n3, n1, mask_13.t().contiguous(), S3, S1, NS)
        ...
        n_heads = 4
        self.attn_1 = SupernodeLinearAttention(C, n_heads, supernode_id_1, n_supernodes)
        self.attn_2 = SupernodeLinearAttention(C, n_heads, supernode_id_2, n_supernodes)
        self.attn_3 = SupernodeLinearAttention(C, n_heads, supernode_id_3, n_supernodes)
        ...
        n_attn = sum(p.numel() for p in [
            *self.attn_1.parameters(), *self.attn_2.parameters(), *self.attn_3.parameters()])
        ...
        print(f"RecurrentBrainNetV1004Bundle: {n_params:,} params total, "
              f"{n_graph:,} graph, {n_attn:,} attn ({n_supernodes} supernodes), "
