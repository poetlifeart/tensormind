...
Usage:
    python -u train_v1004bundle.py --seed 0 --gpu cuda:1 --from-scratch
...
from model_v1004bundle import RecurrentBrainNetV1004Bundle
...
def train_one_epoch(model, loader, optimizer, scheduler, device, epoch,
    ...
    for batch in loader:
        ...
        # model_v1004bundle.py reads, and main() never passed alpha_start /
...
def main():
    ...
    parser.add_argument('--graph', type=str,
                        ...
                        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'graph_8k_parent_supernode.npz'))
    ...
    # ---- ADDED 2026-09-16 (tensor8k) ----
    # Every epoch writes a full ~0.9 GB checkpoint and none are ever removed, so
    # a 50-epoch run costs ~45 GB. That is fine with room to spare and fatal
    # without it. --keep-from N retains every epoch >= N (plus latest.pt and
    # whatever best.json points at) and removes the rest as training proceeds.
    # Default 0 = keep everything = the behaviour of every run to date; the
    # deposited runs are unaffected. Training itself is untouched: this only
    # decides which files survive.
    parser.add_argument('--keep-from', type=int, default=0,
                        help='retain rolling checkpoints from this epoch onward; '
                             'earlier ones are deleted after each epoch. '
                             '0 (default) keeps every epoch, as before. '
                             'The checkpoint-selection rule needs epochs >= 40, '
                             'so --keep-from 38 is safe.')
    ...
    model = RecurrentBrainNetV1004Bundle(args.graph, brain_channels=32, n_iters=args.n_iters).to(device)
    ...
    for name, param in model.named_parameters():
        ...
        #
        # Mechanism: head0/head1/head2 are zero-initialised (model_v1004bundle.py
        ...
        # `a1_base = self.ln1(self.collector(...))` (model_v1004bundle.py:733), so
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # ---- ADDED 2026-09-16 (tensor8k): bounded checkpoint retention ----
        if args.keep_from > 0:
            keep = set()
            bp = os.path.join(args.save_dir, 'best.json')
            if os.path.exists(bp):
                try:
                    with open(bp) as _f:
                        keep.add(json.load(_f)['checkpoint'])
                except Exception:
                    keep = None          # unreadable pointer -> delete nothing
            if keep is not None:
                for _e in range(0, min(epoch, args.keep_from)):
                    _n = f'rolling_ep{_e}_end.pt'
                    if _n in keep:
                        continue
                    _p = os.path.join(args.save_dir, _n)
